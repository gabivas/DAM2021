...
public static final String ADAUGARE_TELEFON="adaugaTelefon";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(intent.hasExtra(MainActivity.EDIT_MOVIE))
        {
            Telefon movie = (Telefon) intent.getSerializableExtra(MainActivity.EDIT_MOVIE);
            etBrand.setText(movie.getModel());
            etDataAparitie.setText(new SimpleDateFormat(FORMAT_DATA, Locale.US).format(movie.getDataAparitie()));
            etModel.setText(movie.getModel());
            etMemorie.setText(""+movie.getMemorie());
            ArrayAdapter<String> adaptor = (ArrayAdapter<String>)spinnerRetea.getAdapter();
            for(int i=0;i<adaptor.getCount();i++)
                if(adaptor.getItem(i).equals(movie.getRetea()))
                {
                    spinnerRetea.setSelection(i);
                    break;
                }
            if(movie.getCuloare().equals("ARGINTIU"))
                radioGroup.check(R.id.radioButton1);
            else if(movie.getCuloare().equals("ROSU"))
